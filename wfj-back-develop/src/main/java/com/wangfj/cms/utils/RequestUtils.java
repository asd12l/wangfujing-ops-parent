...
import static com.wangfj.cms.utils.Constants.POST;
import static com.wangfj.cms.utils.Constants.UTF8;
...
public class RequestUtils {
     ...
     *//*
       ...
       * { String sid = request.getRequestedSessionId(); String ctx =
       * request.getContextPath(); // 如果session id是从url中获取，或者部署路径为空，那么是在正确的。 if
       * (request.isRequestedSessionIdFromURL() || StringUtils.isBlank(ctx)) {
       * return sid; } else { // 手动从cookie获取 Cookie cookie =
       * CookieUtils.getCookie(request, Constants.JSESSION_COOKIE); if (cookie
       * != null) { return cookie.getValue(); } else { return
