...
public abstract class AbstractRedisClient {
    ...
    private synchronized EventLoopGroup getEventLoopGroup(ConnectionPoint connectionPoint) {
        ...
        if (connectionPoint != null && connectionPoint.getSocket() != null) {
            ...
            if (!eventLoopGroups.containsKey(EpollProvider.epollEventLoopGroupClass)) {
                EventLoopGroup epl = clientResources.eventLoopGroupProvider()
                        .allocate(EpollProvider.epollEventLoopGroupClass);
        ...
        try {
            ...
            try {
                ...
                throw new RedisConnectionException(
                        "Could not initialize channel within " + connectionBuilder.getTimeout()
    ...
    public void shutdown(long quietPeriod, long timeout, TimeUnit timeUnit) {
        ...
        if (!sharedResources) {
            ...
            for (EventLoopGroup eventExecutors : eventLoopGroups.values()) {
                Future<?> groupCloseFuture = clientResources.eventLoopGroupProvider()
                        .release(eventExecutors, quietPeriod,
                                timeout,
                                timeUnit);
    ...
    public abstract <K, V> RedisPubSubConnection<K, V> connectPubSub(RedisCodec<K, V> codec);
}
