...
public class LettuceConnection extends AbstractRedisConnection {
    ...
    public Set<byte[]> keys(byte[] pattern) {
        try {
            if (isPipelined()) {
                pipeline(
                        new LettuceResult(getAsyncConnection().keys(pattern), LettuceConverters.bytesListToBytesSet()));
                return null;
            }
            if (isQueueing()) {
                transaction(
                        new LettuceTxResult(getConnection().keys(pattern), LettuceConverters.bytesListToBytesSet()));
                return null;
            }
    ...
    public Boolean getBit(byte[] key, long offset) {
        try {
            if (isPipelined()) {
                pipeline(
                        new LettuceResult(getAsyncConnection().getbit(key, offset), LettuceConverters.longToBoolean()));
                return null;
            }
            if (isQueueing()) {
                transaction(
                        new LettuceTxResult(getConnection().getbit(key, offset), LettuceConverters.longToBoolean()));
                return null;
            }
        ...
        try {
            if (isPipelined()) {
                pipeline(new LettuceResult(
                        getAsyncConnection().linsert(key, LettuceConverters.toBoolean(where), pivot, value)));
                return null;
            }
            if (isQueueing()) {
                transaction(new LettuceTxResult(
                        getConnection().linsert(key, LettuceConverters.toBoolean(where), pivot, value)));
                return null;
            }
    ...
    public Boolean zAdd(byte[] key, double score, byte[] value) {
        try {
            if (isPipelined()) {
                pipeline(new LettuceResult(getAsyncConnection().zadd(key, score, value),
                        LettuceConverters.longToBoolean()));
                return null;
            }
            if (isQueueing()) {
                transaction(new LettuceTxResult(getConnection().zadd(key, score, value),
                        LettuceConverters.longToBoolean()));
                return null;
            }
    ...
    public Long zAdd(byte[] key, Set<Tuple> tuples) {
        try {
            if (isPipelined()) {
                pipeline(new LettuceResult(
                        getAsyncConnection().zadd(key, LettuceConverters.toObjects(tuples).toArray())));
                return null;
            }
            if (isQueueing()) {
                transaction(
                        new LettuceTxResult(getConnection().zadd(key, LettuceConverters.toObjects(tuples).toArray())));
                return null;
            }
    ...
    public Set<Tuple> zRangeByScoreWithScores(byte[] key, Range range, Limit limit) {
        ...
        try {
            if (isPipelined()) {
                if (limit != null) {
                    pipeline(new LettuceResult(
                            getAsyncConnection().zrangebyscoreWithScores(key, min, max, limit.getOffset(),
            ...
            if (isQueueing()) {
                if (limit != null) {
                    transaction(new LettuceTxResult(
                            getConnection().zrangebyscoreWithScores(key, min, max, limit.getOffset(),
                                    limit.getCount()), LettuceConverters.scoredValuesToTupleSet()));
                } else {
            ...
            if (limit != null) {
                return LettuceConverters
                        .toTupleSet(getConnection().zrangebyscoreWithScores(key, min, max, limit.getOffset(),
                                limit.getCount()));
            }
    ...
    public Set<Tuple> zRevRangeByScoreWithScores(byte[] key, Range range, Limit limit) {
        ...
        try {
            if (isPipelined()) {
                if (limit != null) {
                    pipeline(new LettuceResult(
                            getAsyncConnection().zrevrangebyscoreWithScores(key, max, min, limit.getOffset(),
                                    limit.getCount()), LettuceConverters.scoredValuesToTupleSet()));
                } else {
            ...
            if (isQueueing()) {
                if (limit != null) {
                    transaction(new LettuceTxResult(
                            getConnection().zrevrangebyscoreWithScores(key, max, min, limit.getOffset(),
                                    limit.getCount()), LettuceConverters.scoredValuesToTupleSet()));
                } else {
        ...
        try {
            ...
            if (isPipelined()) {
                pipeline(new LettuceResult(getAsyncConnection().eval(convertedScript,
                        ...
                        new LettuceEvalResultsConverter<T>(
                                returnType)));
                return null;
            }
            if (isQueueing()) {
	...
	*/
    ...
    private void validateCommand(CommandType cmd, byte[]... args) {
        ...
        if (!RedisCommand.UNKNOWN.equals(redisCommand) && redisCommand.requiresArguments()) {
            try {
                ...
                throw new InvalidDataAccessApiUsageException(String.format("Validation failed for %s command.", cmd),
                        e);
            }
        }
    }
        ...
        try {
            if (isPipelined()) {
                if (values.length == 1) {
                    ...
                    pipeline(new LettuceResult(
                            getAsyncConnection().pfadd(key, values[0], LettuceConverters.subarray(values, 1))));
                }
                return null;
            }
            if (isQueueing()) {
                if (values.length == 1) {
                    ...
                    transaction(new LettuceTxResult(
                            getConnection().pfadd(key, values[0], LettuceConverters.subarray(values, 1))));
                }
        ...
        try {
            if (isPipelined()) {
                if (keys.length == 1) {
                    ...
                    pipeline(new LettuceResult(
                            getAsyncConnection().pfcount(keys[0], LettuceConverters.subarray(keys, 1))));
                }
                return null;
            }
            if (isQueueing()) {
                if (keys.length == 1) {
                    ...
                    transaction(
                            new LettuceTxResult(getConnection().pfcount(keys[0], LettuceConverters.subarray(keys, 1))));
                }
    ...
    public void migrate(byte[] key, RedisNode target, int dbIndex, MigrateOption option, long timeout) {
        ...
        try {
            if (isPipelined()) {
                pipeline(
                        new LettuceResult(getAsyncConnection().migrate(target.getHost(), target.getPort(), key, dbIndex,
                                timeout)));
                return;
            }
            if (isQueueing()) {
                transaction(
                        new LettuceTxResult(getConnection().migrate(target.getHost(), target.getPort(), key, dbIndex,
                                timeout)));
                return;
            }
