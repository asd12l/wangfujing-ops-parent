...
(function($) {
	function init(param, obj) {
		function getParam() {
			...
			//针对商品管理做的调整
			if($("#cache").val()==1){
			    opts.ajax.param="cache=1";
			    $("#cache").val(0);
			    return opts.ajax.param && (a += "&" + opts.ajax.param),
			    a
			}else{
			    var dataParam = "";
			    dataParam = opts.ajax.param ;
			    if(dataParam != false){
			    	opts.ajax.param = dataParam.replace("cache=1", "cache=0");
			    }
			    return opts.ajax.param && (a += "&" + opts.ajax.param),
			    a
			}
		...
		function getPanelTipInfo() {
			...
			if (opts.panel.tipInfo_on) {
				...
				if ( - 1 != c.indexOf("{input}")) {
					var d = "<input type='text' onpaste='return false' value='" + opts.currPage + "' >";
					c = c.replace("{input}", d),
