...
$(function(){
		...
		//图片上传 没用
		if(attr_image_url==undefined || attr_image_url=="" || attr_image_url=="null"){
			$("#attr_image_url").val("");
		}else{
		...
		//上层图片
		if(attr_image_uppict==undefined || attr_image_uppict=="" || attr_image_uppict=="null"){
		...
		//背景图片
		if(attr_image_backpict==undefined || attr_image_backpict=="" || attr_image_backpict=="null"){
			$("#attr_image_backpict").val("");
		}else{
			$("#img_attr_image_backpict").attr('src', imgServer + attr_image_backpict); 
			$("#attr_image_backpict").val(attr_image_backpict);
		}

		//链接地址
		if(attr_image_link==undefined || attr_image_link=="" || attr_image_link=="null"){
			$("#attr_image_link").val("");
		}else{
		...
		if(attr_flash_url==undefined || attr_flash_url=="" || attr_flash_url=="null"){
			$("#attr_flash_url").val("");
		}else{
  	...
  	function upLoadImg(fileElementId){
  		...
  		$.ajax({
			...
			success : function(response) {
				...
				uploadImg(__ctxPath, ip, username, password, port, path, siteName, fileElementId);
				return;
		  		 $.ajaxFileUpload({
		  			 contentType: "application/x-www-form-urlencoded;charset=utf-8",
			...
			success : function(response) {
				...
				var path=response.path;
		  		 $.ajaxFileUpload({
		  	         ...
		  	         success: function (data) {
		  	             ...
		  	             if(data.success=="true"){
		  	            	...
		  	            	$("#flashPath1").val(data.url);
		  	            	 ...
		  	            	 str = "<span class='flash_error'>"+data.data+"</span>";
		  	            	$("#input_flash"+param).val("");
		  	           	 }
		  	             $("#msgf"+param).html(str);
			  		...
			  		error: function (data, status, e)//服务器响应失败处理函数
			        {
			  			 $("#msgf"+param).html("<span class='img_error'>系统错误，上传失败</span>");
			  			 $("#msgf"+param).removeClass('hide');
			  			$("#input_flash"+param).val("");
			        }
		  	     });
...
//  		}else{
//  			$("#model-body-warning").html("<div class='alert alert-warning fade in'><i class='fa-fw fa fa-times'></i><strong>该版位有广告启用,请检查后再试！</strong></div>");
//			$("#modal-warning").attr({"style":"display:block;","aria-hidden":"false","class":"modal modal-message modal-warning"});
//			
//  		}
  		}
