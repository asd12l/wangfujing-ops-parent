...
function reloadjs(){
	...
	function olvQuery(){
		...
		LA.log('sale2Order-search', '已签收订单查询2', userName,  sessionId);
	...
	//查询销售单(已签收)
 	function initOlv() {
		var url = __ctxPath+"/oms/selectSaleList3";
...
//		if ($("#spanTd12_"+obj).attr("class") == "expand-collapse click-expand glyphicon glyphicon-plus") {
	...
	function trClick(saleNo,orderNo,obj){
		...
		if ($("#orderNo_"+saleNo).attr("class") == "expand-collapse click-expand glyphicon glyphicon-plus") {
			...
			"<th width='2%' style='text-align: center;'>可退货数量</th>"+
					...
					if(response.success=='true'){
						...
						for(var i=0;i<result.length;i++){
							...
							if(ele.supplyProductNo=="[object Object]"||ele.supplyProductNo==undefined){
								option+="<td id='supplyProductNo_"+ele.salesItemNo+"' align='center'></td>";
							}else{
								option+="<td id='supplyProductNo_"+ele.salesItemNo+"' align='center'>"+ele.supplyProductNo+"</td>";
							}
							//标准价
							...
							if(ele.salePrice=="[object Object]"||ele.salePrice==undefined){
								option+="<td id='salePrice_"+ele.salesItemNo+"' align='center'></td>";
							}else{
								option+="<td id='salePrice_"+ele.salesItemNo+"' align='center'>"+ele.salePrice+"</td>";
							}
							//销售数量
							if(ele.saleSum=="[object Object]"||ele.saleSum==undefined){
								option+="<td id='saleSum_"+ele.salesItemNo+"' align='center'></td>";
							}else{
								option+="<td id='saleSum_"+ele.salesItemNo+"' align='center'>"+ele.saleSum+"</td>";
							}
							//是否为赠品
							if(ele.isGift=="0"){
							...
							if(ele.allowRefundNum=="[object Object]"||ele.allowRefundNum==undefined){
								option+="<td id='allowRefundNum_"+ele.saleItemNo+"' align='center'></td>";
							}else{
								option+="<td id='allowRefundNum_"+ele.saleItemNo+"' align='center'>"+ele.allowRefundNum+"</td>";
							}*/
							//订单号
							if(ele.orderNo=="[object Object]"||ele.orderNo==undefined){
								option+="<td align='center'></td>";
							...
							/* //订单明细号
							if(ele.orderItemNo=="[object Object]"||ele.orderItemNo==undefined){
								option+="<td id='saleItemNo' align='center'></td>";
							}else{
								option+="<td id='saleItemNo' align='center'>"+ele.orderItemNo+"</td>";
							} */
							//行号
							...
							if(ele.refundNum=="[object Object]"||ele.refundNum==undefined){
								option+="<td id='refundNum_"+ele.salesItemNo+"' align='center'></td>";
							}else{
								option+="<td id='refundNum_"+ele.salesItemNo+"' align='center'>"+ele.refundNum+"</td>";
							}
							//提货数量
...
//		callCenterComments = callCenterComments.replace(/[\r\n]/g, ""); 
		$.ajax({
			...
			data : {
				...
				"refundPcitureUrl":refundPcitureUrl
			},
			dataType : "json",
	...
	function successBtn(){
		...
		$("#pageBody").load(__ctxPath+"/jsp/saleOrderListView2.jsp");
	}
	...
	/*图片上传*/
  	function upLoadImg(param){
  		...
  		$("#input_brand"+param).val("");
  		 ...
  		 $.ajaxFileUpload({
  	         url:__ctxPath+"/orderUpImg/uploadOms-noMulti",
  	         type: "POST",
  	         secureuri: true,
  	         ...
  	         success: function (data) {
  	             ...
  	             if(data.success=="true"){
  	            	...
  	            	$("#model-body-warning").html("<div class='alert alert-warning fade in'><i class='fa-fw fa fa-times'></i><strong>"+data.data+"</strong></div>");
 	     	  		$("#modal-warning").attr({"style":"display:block;z-index:2000","aria-hidden":"false","class":"modal modal-message modal-warning"});
                        ...
                        <div class="col-xs-12 col-md-12">
                            <div class="widget">
                                <div class="widget-header ">
                                    <h5 class="widget-caption">已签收退货管理</h5>
					            ...
					            <div style="width:100%;height:420px; overflow:scroll; ">
					                    ...
					                    <div>
					                     ...
					                     <div class="form-group" style="height:90px">
											<label class="col-lg-2 col-sm-3 col-xs-3 control-label">图片：</label>
											<div class="col-lg-2 col-sm-3 col-xs-3 control-label">
												<input onchange="upLoadImg('2')" type="file" id="brandimg2" name="brandimg2" accept=".jpg,.gif,.png"/>
												<!-- <a style="width: 20;" class="showImgNameA" id="showImgNameA2" href="javascript:show(2);">未选择图片</a> -->
