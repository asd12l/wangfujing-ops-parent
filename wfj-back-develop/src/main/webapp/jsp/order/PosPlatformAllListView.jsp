...
/* 	saleMsgImage="http://172.16.200.4/images"; */
	...
	function olvQuery(){
		$("#businessClass_form").val($("#refundClass_select").val());
	...
	function reset(){
		$("#refundClass_select").val("saleOrder");
		$("#payType_select").val("");
		$("#reservation").val("");
		olvQuery();
	}
	//初始化包装单位列表
 	function initOlv() {
		var url = __ctxPath+"/omsOrder/posPlatFormAllList";
...
//							option3+="<td align='center'>"+ele.deleteFlag+"</td>";
	...
	function exportExecel() {
		var paymentType = $("#payType_select").val();
		...
		var title = "PosPlatformAll";
		var count = $("#olv_tab tbody tr").length;
		console.log(count);
		if (count > 0
				/* && (refundClass !="" || paymentType != "" || startTime != "" || endTime != "") */) {
			window.open(__ctxPath + "/omsOrder/getPosPlatformAllToExcel?paymentType=" + paymentType + "&&startTime=" + startTime + "&&endTime=" + endTime
					+ "&&refundClass=" + refundClass + "&&title=" + title);
		} else {
			$("#model-body-warning")
					.html(
                        ...
                        <div class="col-xs-12 col-md-12">
                            <div class="widget">
                                <div class="widget-header ">
                                    <h5 class="widget-caption">支付平台报表汇总</h5>
                                ...
                                <div class="widget-body" id="pro">
                                    <div class="table-toolbar">
                                    		<ul class="topList clearfix">                           			
                                    			<li class="col-md-4">
                                    				<label class="titname">时&nbsp;&nbsp;间：</label>
                                    				<input type="text" id="reservation" />
                                   				</li>
                                    			<li class="col-md-4">
                                    					<label class="titname">业务类型：</label>
	                                    				<select id=refundClass_select style="padding:0 0;">
				                                			<option value="saleOrder" selected="selected">销售订单</option>
                                    ...
                                    <table class="table-striped table-hover table-bordered" id="olv_tab" style="width: 100%;background-color: #fff;margin-bottom: 0;">
                                        <thead>
                                            <tr role="row" style='height:35px;'>
                                                <th width="3%" style="text-align: center;">业务类型</th>
                                                <th width="3%" style="text-align: center;">支付方式</th>
                                                <th width="2%" style="text-align: center;">应收金额(元)</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                        </tbody>
									...
									<form id="olv_form" action="">
										...
										&nbsp; 
										<input type="hidden" id="businessClass_form" name="refundClass"/>
