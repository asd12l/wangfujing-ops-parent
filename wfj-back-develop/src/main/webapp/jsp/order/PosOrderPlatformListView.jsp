...
/* 	saleMsgImage="http://172.16.200.4/images"; */
	...
	$(function() {
		/* //支付方式
		$("#payType_select").one("click",function(){
			$.ajax({
				type: "post",
	...
	function olvQuery(){
		...
		$("#payType_form").val($("#payType_select").val());
		...
		if(strTime2!=""){
			...
			$("#refundStartFlowTime_form").val(strTime2[0].replace("/","-").replace("/","-"));
			$("#refundEndFlowTime_form").val(strTime2[1].replace("/","-").replace("/","-"));
		}else{
			$("#refundStartFlowTime_form").val("");
			$("#refundEndFlowTime_form").val("");
		}
	...
	//初始化包装单位列表
 	function initOlv() {
		var url = __ctxPath+"/omsOrder/orderCancelRefundMoney";
...
//		$("#imageDiv").text(ur);
	...
	//点击tr事件
	function trClick(orderNo,isCod,obj){
...
//							option3+="<td align='center'>"+ele.deleteFlag+"</td>";
		...
		"<th width='5%' style='text-align: center;'>销售单号</th>"+
		"<th width='5%' style='text-align: center;'>积分增减</th>"+
				...
				if(response.success=='true'){
					...
					for(var i=0;i<result.length;i++){
						...
						if(ele.customerAccount=="[object Object]"||ele.customerAccount==undefined){
							...
							option8+="<tr style='height:35px;overflow-X:hidden;'><td align='center'>"+ele.customerAccount+"</td>";
						}
						//积分增减
						if(ele.customerLevel=="[object Object]"||ele.customerLevel==undefined){
							option8+="<td align='center'></td>";
						}else{
							option8+="<td align='center'>"+ele.customerLevel+"</td>";
						}
						//操作时间
						if(ele.receptName=="[object Object]"||ele.receptName==undefined){
							option8+="<td align='center'></td>";
						}else{
							option8+="<td align='center'>"+ele.receptName+"</td>";
						}
					}
				}
			}
	...
	function exportExecel() {
		var payType = $("#payType_select").val();
		...
		if(strTime!=""){
			strTime = strTime.split("-");
			startTime = strTime[0].replace("/","-").replace("/","-");
			endTime = strTime[1].replace("/","-").replace("/","-");
		}else{
			startTime = $("#startSaleTime_form").val();
			endTime = $("#endSaleTime_form").val();
		}
		var strTime2 = $("#reservation2").val();
		var endTime;
		var refundStartFlowTime;
		if(strTime2!=""){
			strTime2 = strTime2.split("-");
			refundStartFlowTime = strTime2[0].replace("/","-").replace("/","-");
			refundEndFlowTime = strTime2[1].replace("/","-").replace("/","-");
		}else{
			refundStartFlowTime = $("#refundStartFlowTime_form").val();
			refundEndFlowTime = $("#refundEndFlowTime_form").val();
		}
		var title = "PosOrderPlatform";
		var count = $("#olv_tab tbody tr").length;
		console.log(count);
		if (count > 0
				/* && (saleNo !="" || orderNo !="" || payType != "" || startTime != "" || endTime != "" || refundStartFlowTime != "" || refundEndFlowTime != "") */) {
			window.open(__ctxPath + "/omsOrder/getPosOrderPlatformToExcel?payType=" + payType + "&&startTime=" + startTime + "&&endTime=" + endTime
					+ "&&saleNo=" + saleNo + "&&orderNo=" + orderNo + "&&title=" + title + "&&refundEndFlowTime=" + refundEndFlowTime + "&&refundStartFlowTime=" + refundStartFlowTime);
		} else {
			$("#model-body-warning")
					.html(
                        ...
                        <div class="col-xs-12 col-md-12">
                            <div class="widget">
                                <div class="widget-header ">
                                    <h5 class="widget-caption">订单取消扣款报表</h5>
                                ...
                                <div class="widget-body" id="pro">
                                    <div class="table-toolbar">
                                    		<ul class="topList clearfix">                           			
                                    			<li class="col-md-4">
                                    				<label class="titname">下单时间：</label>
                                    			...
                                    			<li class="col-md-4">
                                    					<label class="titname">扣款类型：</label>
	                                    				<select id=payType_select style="padding:0 0;">
				                                			<option value="" selected="selected">全部</option>
				                                			<option value="JFKH">积分扣回</option>
				                                			<option value="ZKKH">折扣扣回</option>
				                                			<option value="YQKH">用券扣回</option>
				                                			<option value="FQKH">返券扣回</option>
				                                			<option value="DKKH">抵扣扣回</option>
				                                			<option value="YFKU">运费扣回</option>
				                                			<option value="YJKF">积分消费扣回</option>
				                                		</select>
                                    				</li>
                                    				<li class="col-md-4">
                                    				<label class="titname">订单取消时间：</label>
                                    ...
                                    <table class="table-striped table-hover table-bordered" id="olv_tab" style="width: 100%;background-color: #fff;margin-bottom: 0;">
                                        <thead>
                                            <tr role="row" style='height:35px;'>
                                                <th width="3%" style="text-align: center;">订单取消日期</th>
                                                <th width="3%" style="text-align: center;">下单日期</th>
                                                <th width="2%" style="text-align: center;">订单号</th>
                                                <th width="2%" style="text-align: center;">销售单号</th>
                                                <th width="2%" style="text-align: center;">应退款方式</th>
                                                <th width="2%" style="text-align: center;">应退款金额</th>
                                                <th width="2%" style="text-align: center;">实际退款金额</th>
                                                <th width="2%" style="text-align: center;">退货扣款金额</th>
                                                <th width="2%" style="text-align: center;">扣款原因</th>
                                                <th width="2%" style="text-align: center;">扣款类型</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                        </tbody>
									...
									<form id="olv_form" action="">
										...
										<input type="hidden" id="refundStartFlowTime_form" name="refundStartFlowTime"/>
										<input type="hidden" id="refundEndFlowTime_form" name="refundEndFlowTime"/>
									</form>
								</div>
											...
											{#foreach $T.list as Result}
												<tr class="gradeX" id="gradeX{$T.Result.sid}" style="height:35px;">
													...
													<td align="center" id="refundAmount_{$T.Result.sid}">
														{#if $T.Result.refundAmount > 0}-{$T.Result.refundAmount}
														{#elseif $T.Result.refundAmount <= 0}{$T.Result.refundAmount}
						                   				{#/if}
													</td>
													<td align="center" id="needRefundAmount_{$T.Result.sid}">
						                   				{#if $T.Result.needRefundAmount > 0}-{$T.Result.needRefundAmount}
														{#elseif $T.Result.needRefundAmount <= 0}{$T.Result.needRefundAmount}
						                   				{#/if}
													</td>
													...
													<td align="center" id="payname_{$T.Result.sid}">
														{#if $T.Result.payname != '[object Object]'}{$T.Result.payname}
						                   				{#/if}
													</td>
													<td align="center" id="payType_{$T.Result.sid}">
														{#if $T.Result.payType == 'ZKKH'}折扣扣回
						                      			{#elseif $T.Result.payType == 'YFKU'}运费扣回
						                      			{#elseif $T.Result.payType == 'DKKH'}抵扣扣回
						                      			{#elseif $T.Result.payType == 'FQKH'}返券扣回
						                      			{#elseif $T.Result.payType == 'JFKH'}积分扣回
						                      			{#elseif $T.Result.payType == 'YQKH'}用券扣回
						                      			{#elseif $T.Result.payType == 'YJKF'}积分消费扣回
						                   				{#/if}
													</td>
									       		</tr>
											{#/for}
                                        ...
                                        <thead>
                                            <tr role="row" style='height:35px;'>
                                               <th width="3%" style="text-align: center;">订单号</th>
                                                ...
                                                <th width="2%" style="text-align: center;">订单金额</th>
                                                <th width="3%" style="text-align: center;">支付成功日期</th>
                                                <th width="3%" style="text-align: center;">支付对账日期</th>
                                                <th width="3%" style="text-align: center;">付款方式</th>
                                            </tr>
                                        </thead>
                                        <tbody id="mainTr">
                                        </tbody>
