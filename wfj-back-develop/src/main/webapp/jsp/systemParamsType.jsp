...
<title>公告管理</title>
<!--  
<script type="text/javascript" src="${ctx}/system_parameter/systemParamTypeView.js"></script>
<script type="text/javascript" src="${ctx}/system_parameter/addSysParamType.js"></script>
<script type="text/javascript" src="${ctx}/system_parameter/updateSysParamType.js"></script>
-->
...
//初始化商品列表
	function initBt() {
	var url = __ctxPath + "/sysParameterType/selectByCode";
...
function modifyBt(){
	...
	$("#sid").val(value);
	$("#code").val($("#code_"+value).text().trim());
	$("#type").val($("#type_"+value).text().trim());
	$("#divTitle").html("修改系统参数类别");
	$("#btDiv").show();
}
function tab(data){
	if(data=='pro'){//基本
	...
	bootbox.confirm("确定删除吗？", function(r){
		if(r){
			$.ajax({
				...
				url: __ctxPath+"/sysParameterType/deleteByPrimaryKey",
				dataType: "json",
				data: {
					"sid":value
				},
...
function saveDivFrom(){
	var url = "";
	if($("#sid").val()!=""){
		url=__ctxPath + "/sysParameterType/update";
	}else{
		url=__ctxPath+"/sysParameterType/save";
	}
	$.ajax({
		type: "post",
		contentType: "application/x-www-form-urlencoded;charset=utf-8",
...
function successBtn(){
	...
	$("#pageBody").load(__ctxPath+"/jsp/systemParamsType.jsp");
}
function closeBtDiv(){
	$("#btDiv").hide();
}
                        ...
                        <div class="col-xs-12 col-md-12">
                            <div class="widget">
                                <div class="widget-header ">
                                    <span class="widget-caption"><h5>系统参数类别</h5></span>
								...
								<p style="display:none">
									<textarea id="bt-list" rows="0" cols="0">
										<!--
										{#template MAIN}
											{#foreach $T.sysParameterTypeList as Result status}
												<tr class="gradeX">
													<td align="left">
														<div class="checkbox">
															<label>
													...
													<td align="center" id="type_{$T.Result.sid}">{$T.Result.type}</td>
									       		</tr>
											{#/for}
									    {#/template MAIN}	-->
									</textarea>
				            ...
				            <div class="col-md-12">
				            	<form id="divForm" method="post" class="form-horizontal" enctype="multipart/form-data">
					                ...
					                <div class="form-group">
					 					编码：
					 					<input type="text" placeholder="必填" class="form-control" id="code" name="code">
					                </div>
					                <div class="form-group">
										类别：
										<input type="text" placeholder="必填" class="form-control" id="type" name="type">
					                </div>
				                </form>
				            </div>
				        </div>
