...
<script type="text/javascript">
	...
	function initOlv() {
		var url = __ctxPath + "/statement/shopSaleController";
	...
	function exportExcel(){
		...
		var title = "SHOPIN"+saleTimeFrom+"To"+saleTimeTo+"SHOPSALE";
		var count =  $("#olv_tab tbody tr").length;
		if(count>0&&saleTimeFrom!=""&&saleTimeTo!=""){
			window.open(__ctxPath+"/stateExcel/shopSaleToExcel?saleTimeFrom="+saleTimeFrom+"&&saleTimeTo="+saleTimeTo+"&&title="+title);
		}else{
	...
	function exportPDF(){
		...
		var count =  $("#olv_tab tbody tr").length;
		if(count>0&&saleTimeFrom!=""&&saleTimeTo!=""){
			window.open(__ctxPath+"/statePDF/shopSaleToPDF?saleTimeFrom="+saleTimeFrom+"&&saleTimeTo="+saleTimeTo+"&&title="+title);
		}else{
					...
					<div class="col-xs-12 col-md-12">
						<div class="widget">
							<div class="widget-header ">
								<span class="widget-caption"><h5>门店销售统计管理</h5>
								</span>
									...
									<table class="table-striped table-hover table-bordered"
										...
										<thead>
											<tr role="row">
												...
												<th width="5%" style="text-align: center;">销售金额</th>
												<th width="5%" style="text-align: center;">销售数量</th>
												<th width="5%" style="text-align: center;">单价</th>
												<th width="5%" style="text-align: center;">退货金额</th>
												<th width="5%" style="text-align: center;">退货数量</th>
												<th width="5%" style="text-align: center;">退货单价</th>
											</tr>
										</thead>
										<tbody>
										</tbody>
										...
										{#template MAIN}
											{#foreach $T.list as Result}
												<tr class="gradeX" id="gradeX{$T.Result.sid}" onclick="trClick({$T.Result.sid})">
													...
													<td align="center" id="shopSaleAvgPrice_{$T.Result.sid}">{$T.Result.shopSaleAvgPrice}</td>
													...
													<td align="center" id="shopRefundAvgPrice_{$T.Result.sid}">{$T.Result.shopRefundAvgPrice}</td>
									       		</tr>
											{#/for}
									    {#/template MAIN}	-->
									</textarea>
