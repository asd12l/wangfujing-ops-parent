...
<script type="text/javascript">
	...
	function manual(){
        ...
        if(numiid == ""){
        	...
        	$.ajax({
        		type: "GET",
     			url : __ctxPath + "/ediGoods/goodsManual?numiid="+numiid+"&channelCode=CB",
     			dataType : "json",
	...
	//初始化包装单位列表
 	function initOlv() {
		var url = __ctxPath+"/ediGoods/selectGoodsList?channel_code=CB";
	...
	//批量关联
 	function batchAssociated(){
 		...
 		if(suffix == 'xlsx' || suffix == 'XLSX' || suffix == 'xls' || suffix == 'XLS'){
 			var url = __ctxPath + "/ediGoods/goodsBatch?channelCode=CB";
	...
	function again(){
		var url = __ctxPath + "/jsp/edi/QQG/goodsrelation.jsp";
		$("#pageBody").load(url);
	}
	...
	function automatic(){
       	 $.ajax({
       		on: true,
  			url : __ctxPath + "/ediGoods/goodsAutomatic?channelCode=CB",
  			dataType : "json"
    	});
        reset();
   	}
	...
	function removeRelation(outerid,numiid){
       	 $.ajax({
       		on: true,
    			url : __ctxPath + "/ediGoods/goodsRemove?outerid="+outerid+"&numiid="+numiid+"&channelCode=CB",
    			dataType : "json",
    			success : function(data) {
		            reset();
				},
					...
					<div class="col-xs-12 col-md-12">
						<div class="widget">
							<div class="widget-header ">
								<h5 class="widget-caption">全球购商品关联</h5>
							...
							<div class="widget-body" id="pro">
								...
								<table class="table table-bordered table-striped table-condensed table-hover flip-content" id="goods_table">
                                       <thead class="flip-content bordered-darkorange">
										<tr role="row">
											<th style="text-align: center;">全球购ID</th><!-- num_iid -->
