...
<script type="text/javascript">
	...
	function zTreeOnClick(e, treeId, treeNode) {
		...
		$.ajax({
			type : "post",
			url : __ctxPath + "/receiptAddress/queryAddressInfo?channel=M4",
			data : _requestData,
			dataType : "json",
			success : function(resJSON) {
				...
				var url = __ctxPath
						+ "/receiptAddress/queryWFJProperties?type=TMALL_AREA&channel=M4";
				$.ajax({
					type : "post",
					url : url,
					dataType : "json",
	...
	var _setting = {
		...
		async : {
			enable : true,
			url : __ctxPath + "/receiptAddress/queryAddress?channel=M4",
	...
	var _updateParam = function(callback) {
		...
		LA.env = 'dev';
		  ...
		  LA.log('yz-address-modify', '有赞收货地址修改', userName, sessionId);
		...
		$.ajax({
			type : "post",
			url : __ctxPath + "/receiptAddress/updateAddress?channel=M4",
			data : _requestData,
			dataType : "json",
					...
					<div class="col-xs-12 col-md-12">
						<div class="widget">
							<div class="widget-header ">
								<h5 class="widget-caption">有赞地址管理</h5>
