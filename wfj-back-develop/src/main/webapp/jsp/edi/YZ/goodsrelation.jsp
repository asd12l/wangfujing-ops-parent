...
<script type="text/javascript">
    ...
    function stockChange(outerid) {
    	...
    	var priceLine; 
	 	$.ajax({  
	    	url: __ctxPath + "/ediGoods/selectCommoditySearch?outer_id=" + outerid + "&channelCode=M4",  
	    	cache:false,
	    	dataType:"json",  
	...
	//初始化包装单位列表
 	function initOlv() {
		var url = __ctxPath+"/ediGoods/selectGoodsList?channel_code=M4";
	...
	function manual(){
        ...
        if(numiid){
        	 $.ajax({
        		on: true,
     			url : __ctxPath + "/ediGoods/goodsManual?numiid="+numiid+"&channelCode=M4",
     			dataType : "json",
     			success : function(data) {
     				reset();
				},
	...
	//批量关联
 	function batchAssociated(){
 		...
 		var suffix = fileA[fileA.length-1];
 	 	var url = __ctxPath + "/ediGoods/goodsBatch?channelCode=M4";
	...
	function automatic(){
       	 $.ajax({
       		on: true,
  			url : __ctxPath + "/ediGoods/goodsAutomatic?channelCode=M4",
  			dataType : "json"
    	});
        reset();
   	}
					...
					<div class="col-xs-12 col-md-12">
						<div class="widget">
							<div class="widget-header ">
								<h5 class="widget-caption">有赞商品关联</h5>
							...
							<div class="widget-body" id="pro">
								...
								<table class="table table-bordered table-striped table-condensed table-hover flip-content" id="goods_table">
                                       <thead class="flip-content bordered-darkorange">
										<tr role="row">
											<th style="text-align: center;">有赞ID</th><!-- num_iid -->
											...
											{#foreach $T.list as Result}
												<tr style="height: 10px" class="gradeX">
													...
													<td align="center" id="productCode_{$T.Result.sid}">
														{#if $T.Result.brand_name == null || $T.Result.brand_name == ""} --- {#else} {$T.Result.brand_name} {#/if}
													</td>
