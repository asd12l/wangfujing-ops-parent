			...
			success : function(response) {
				if(response.success=="true"){
					...
					if(response.data.length!=0){
						mes="<div style='text-align:left;font-size:11px'>";
						...
						$("#warning2Body").text(buildErrorMessage("",mes));
						$("#warning2").show();
					}
					productQuery();
				}
...
<script type="text/javascript">
	function updateProduct() {
		...
		$("input[type='checkbox']:checked").each(function(i, team) {
			var productSid = $(this).val();
			var skuSale=$("#skuSale_"+productSid).html().trim();
			var photoStatus=$("#photoStatus_"+productSid).html().trim();
			if(skuSale=="1"){
				count++;
			}
			/* if(photoStatus!="4"){
					...
					<div class="col-xs-12 col-md-12">
						<div class="widget">
							<div class="widget-header ">
								<h5 class="widget-caption">商品上下架管理</h5>
										...
										{#template MAIN}
											{#foreach $T.list as Result}
												<tr class="gradeX">
													...
													<td style="display:none;" id="photoStatus_{$T.Result.sid}">{$T.Result.photoStatus}</td>
									       		</tr>
											{#/for}
									    {#/template MAIN}	-->
									</textarea>
