...
<script type="text/javascript">
	function deleteProduct() {
		...
		if (checkboxArray.length > 1) {
			...
			$("#warning2Body").text("请选取要停用的商品!");
			$("#warning2").show();
			return false;
		}
		var value = checkboxArray[0];
		var proActiveBit=$("#proActiveBit_"+value).html().trim();
		if(proActiveBit=="0"){
			$("#warning2Body").text("该商品已停用");
			$("#warning2").show();
			return false;
		}
		$.ajax({
	...
	function editProductSX() {
		...
		for(var i = 0; i<dataList.length; i++){//alert(dataList[i].sid);
			if(productChangePropId == dataList[i].sid){
				productDetail = dataList[i];
				break;
			}
		}
		
		skuCode = $("#skuCode_" + productChangePropId).html().trim();
		category_Sid = $("#category_" + productChangePropId).html().trim();
		category_Name = $("#categoryName_" + productChangePropId).html().trim();
		statcate_Name = $("#statCategoryName_" + productChangePropId).html()
				.trim();
		productSid = $("#spuCode_" + productChangePropId).html().trim();
		productSid2 = $("#spuSid_" + productChangePropId).html().trim();
		tabMark = "base";
		editProduct(productChangePropId, skuCode);
		/* var url = __ctxPath + "/jsp/product/editProduct.jsp";
		$("#pageBody").load(url); */
	}
</script>
<!-- 操作 -->
...
<script type="text/javascript">
	function getView(data) {
		...
		$("#pageBody").load(url, {
			"backUrl" : "/jsp/product/ProductView.jsp"
		}, function(){
			$(".loading-container").addClass("loading-inactive");
		});
	}
	
	function editProduct(data, skuCode) {
		var url = __ctxPath + "/jsp/product/editProduct.jsp";
		$("#pageBody").load(url);
	}
</script>
<script type="text/javascript">
	...
	function closeTIPS(spuCode,colorSid) {
		/* if(oldSpuCode != spuCode && oldColorSid != colorSid){
			oldSpuCode = "";
			colorSid = "";
			$("#TIPS").hide();
								...
								<div class="table-toolbar">
									<div class="clearfix">
										<a id="editabledatatable_new" onclick="addProduct();"
											class="btn btn-primary"> <i class="fa fa-plus"></i> 添加商品
										</a><!-- &nbsp;&nbsp;&nbsp;&nbsp; <a id="editabledatatable_new"
											onclick="appendProduct();" class="btn btn-primary"> <i
											class="fa fa-plus"></i> 新增专柜商品
										</a> -->&nbsp;&nbsp;&nbsp;&nbsp; <a id="editabledatatable_new"
											onclick="editProductSX();" class="btn btn-primary"> <i
											class="fa fa-edit"></i> 商品修改
										</a>&nbsp;&nbsp;&nbsp;&nbsp;
									</div>
									<div class="mtb10" style="float:left;width:100%;margin-bottom:0">
										...
										<div class="col-md-3" style="margin-bottom: 15px;width:24%">
										    ...
										    <select id="proType_select" style="width: 150px;">
											    <option value="">全部</option>
									...
									<div class="mtb10" style="float:left;width:100%;margin-bottom:0;margin-top:0">
										<div class="col-md-3" style="margin-bottom: 15px;width:24%">
										    <label style="width: 70px;text-align:right;">款号：</label>
										    <input style="width: 150px;" type="text" id="modelCode_input" />
										</div>
										<div class="col-md-3" style="margin-bottom: 15px;width:24%">
										    <label style="width: 70px;text-align:right;">色系：</label>
										    <select style="width: 150px;" id="colorSid_select">
										        <option value="">全部</option>
										    </select>
										</div>
										<div class="col-md-3" style="margin-bottom: 15px;width:24%">
									...
									<div class="mtb10" style="float:left;width:100%;margin-bottom:0;margin-top:0">
										<ul class="listInfo" style="margin-left: 15px;width:24%;">
											<li>
											    ...
											    <!-- <select style="width: 150px;text-align: left;" id="brandGroupCode_select">
											        <option value="" style="text-align: left;">全部</option>
											    </select> -->
											...
											{#foreach $T.list as Result}
												<tr class="gradeX">
													...
													<td align="center" style="text-overflow:ellipsis;word-break:keep-all;overflow:hidden;">
														<a onclick="getView({$T.Result.skuCode});" style="cursor:pointer;">{$T.Result.skuName}</a></td>
													...
													<td align="center" style="cursor:pointer;"
															onmouseover="mouseover(event,{$T.Result.spuCode},{$T.Result.colorSid},{$T.Result.photoStatus});"
															onmouseout="closeTIPS({$T.Result.spuCode},{$T.Result.colorSid});">
