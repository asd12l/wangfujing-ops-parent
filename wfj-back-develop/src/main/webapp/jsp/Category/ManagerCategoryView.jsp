...
// 					    treeObj.reAsyncChildNodes(tarnode, "refresh");
	...
	function refreshAllZTree(){
		$.ajax({
			type : "post",
			contentType : "application/x-www-form-urlencoded;charset=utf-8",
			url : __ctxPath + "/category/listAddPermission",
			async : false,
			dataType : "json",
			data:{
				"cache":"1"
			...
			success : function(response) {
				...
				for(var i=0;i<response.length;i++){
					if(response[i].categoryType==1){
						cateShopCodeList.push(response[i].shopSid);
		...
		$.ajax({
            ...
            success : function(response) {
            	if(response.success == "true"){
            		...
            		for(var jj=0;jj<cateShopCodeList.length;jj++){
            			for (var i = 0; i < result.length; i++) {
            				var ele = result[i];
            				if(cateShopCodeList[jj] == ele.organizationCode){
            					var option = $("<option value='" + ele.organizationCode + "'>" + ele.organizationName + "</option>");
                                option.appendTo($("#shopCode"));
                                break;
            				}
                        }
            		}
					...
					<div class="col-lg-12 col-sm-12 col-xs-12">
						<div class="col-md-4" style="width:40%;float:left;">
	                        <div class="well clearfix" style="padding: 10px;">
	                            <div class="">
	                                <a class="notbtn purple btn-sm fa fa-star" style="width: 99.9%;cursor: default;">&nbsp;管理分类树信息</a>
	                            </div>
	                            &nbsp;
	                            <div class="">
	                                ...
	                                <span>门店：</span>
	                                <select id="shopCode" style="height:26px;width: 165px;margin-bottom: 10px;">
