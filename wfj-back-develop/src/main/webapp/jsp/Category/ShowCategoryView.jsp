...
// 					    treeObj.reAsyncChildNodes(tarnode, "refresh");
	...
	$(function() {
		...
		$("#appGBFLDivChannel").change(getCategoryList);
		
	});
	
	function getCategoryList(){
		var channelSid=$("#appGBFLDivChannel").val();
		$.ajax({
			type : "post",
			contentType : "application/x-www-form-urlencoded;charset=utf-8",
			...
			success : function(response) {
				if(response.success=="true"){
					...
					if(!status){
						...
						$("#hrootstatus").prop("disabled", true);
					} else {
						$("#hrootstatus").prop("checked", true);
						$("#hrootstatus").removeAttr("disabled");
					}
				} else {
					$("#hrootstatus").prop("checked", true);
					$("#hrootstatus").removeAttr("disabled");
		...
		if (treeObj != "") {
			...
			$.ajax({
				...
				success : function(response) {
					...
					if(status==0||(status==1&&treeObj[0].status=="Y")||treeObj[0].clevel!=1){
						$("#status1").removeAttr("disabled");
					...
					.ajax({
						...
						success : function(response) {
							if (response.status == "Y") {
								...
								$("#cateCode2").val(response.categorySid);
					...
					<div class="col-lg-12 col-sm-12 col-xs-12">
						<div class="col-md-4" style="width:40%;float:left;">
	                        <div class="well clearfix" style="padding: 10px;">
	                            <div class="">
	                                <a class="btn btn-default purple btn-sm fa fa-star" style="width: 99.9%;cursor: default;" onclick="appGBFL()">&nbsp;展示分类树信息</a>
	                            </div>
	                            &nbsp;
	                            <div class="">
