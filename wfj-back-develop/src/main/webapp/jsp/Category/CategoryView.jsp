...
<script type="text/javascript">
	...
	function zTreeOnAsyncError(event, treeId, treeNode) {
		$("#warning2Body").text(buildErrorMessage("","异步加载失败！"));
		...
		$.ajax({
			...
			success : function(response) {
				...
				for(var i=0;i<response.length;i++){
					if(response[i].categoryType==0||response[i].categoryType==2){
						zTreeCate.push(response[i]);
					} else if(response[i].categoryType==1){
						response[i].pId = "managerCate";
						zTreeCate.push(response[i]);
					} else if(response[i].categoryType==3){
						response[i].pId = "showCate";
						zTreeCate.push(response[i]);
					}
				}
				zTreeCate.push({
...
// 		}
	...
	function append() {
		...
		if(selectNodes==""){
			$("#model-body-warning")
			.html(
					"<div class='alert alert-warning fade in'><strong>未选中节点!</strong></div>");
        	$("#modal-warning").attr({
			"style" : "display:block;",
			"aria-hidden" : "false",
	...
	function deleteCategory(){
		...
		if(isleaf=="Y"&&cateType==0){
			$.ajax({
				...
				success : function(response) {
					if (response.data >= '1') {
						$("#warning2Body").text(buildErrorMessage("","该品类下已经有商品，不能删除！"));
						$("#warning2").attr("style", "z-index:9999");
						$("#warning2").show();
						flag=true;
					}
...
// 										treeObj.updateNode(treenode);;
	...
	function saveDivFrom2() {
		...
		$("#div2Table :eq(0)").each(
				function(i) {
					var this1 = this;
					$("input[type='checkbox']:checked", this1).each(
							function(j) {
								var propsid = $(this).attr("propsid");
								var check = $("input[name='propsName_"
										+ propsid + "']:checked");
								if (check.length != 0) {
									notNull.push({
										'notNull' : 1,
											...
											success : function(response) {
												...
												for (var i = 0; i < result.length; i++) {
													...
													option += "<tr onchange='div2TrChange()'>"
															+ "<td width='50px' style='vertical-align: text-top;padding: 0px;'>"
															+ "<div class='checkbox'><label>"
															+ "<input type='checkbox' div2TableCheck='on' propsid='"+ele.propsSid+"' value="+ele.propsName+" >"
															+ "<span class='text'></span></label>"
															+ "</div>"
															+ "</td>"
															+ "<td width='100px' style='vertical-align: text-top;padding: 0px;'>"
															+ ele.propsName
															+ "</td>"
															+ "<td width='250px' style='vertical-align: text-top;padding: 0px;'>"
															+ ele.propsDesc
															+ "</td>"
															+ "<td width='89px' style='vertical-align: text-top;padding: 0px;'>"
															+ "<div class='checkbox'><label>"
												...
												$(
														"#div2Table input[div2TableCheck='on']")
														.each(
																function(i) {
				...
				.ajax({
					...
					success : function(response) {
						...
						for (var i = 0; i < result.length; i++) {
							var ele = result[i];
							option += "<tr onchange='div2TrChange()'>"
								+ "<td width='17.5%' style='vertical-align: text-top;padding: 0px;'>"
								+ "<div class='checkbox'><label>"
								+ "<input type='checkbox' div2TableCheck='on' propsid='"+ele.propsSid+"' value="+ele.propsName+" >"
								+ "<span class='text'></span></label>"
								+ "</div>"
								+ "</td>"
								+ "<td width='35.6%' style='vertical-align: text-top;padding: 0px;'>"
								+ ele.propsName
								+ "</td>"
								+ "<td width='35.6%' style='vertical-align: text-top;padding: 0px;'>"
								+ ele.propsDesc
								+ "</td>"
								+ "<td width='50%' style='vertical-align: text-top;padding: 0px;'>"
								+ "<div class='checkbox'><label>"
								+ "<input type='checkbox' onCheck2='ch'  onCheck2Sid='"+ele.propsSid+"' name='propsName_"+ele.propsSid+"'>"
								+ "<span class='text'></span></label>"
								+ "</div>"
						...
						$("#div2Table").append(option);
						$("#div2Table input[type='checkbox']").each(
								function(i) {
									for (var j = 0; j < propsid.length; j++) {
										if (propsid[j] == $(this).attr(
												"propsid")) {
					...
					<div class="col-lg-12 col-sm-12 col-xs-12">
						<div class="col-md-4" style="width:40%;float:left;">
	                        <div class="well clearfix" style="padding: 10px;">
	                            <div class="">
	                                <a class="btn btn-default purple btn-sm fa fa-star" style="width: 99.9%" onclick="appGBFL()">&nbsp;添加分类树信息</a>
	                            </div>
	                            &nbsp;
	                            <div class="">
	                                <a class="btn btn-default purple btn-sm fa fa-plus" id="addbutton" onclick="append()">&nbsp;添加</a>
										...
										{#template MAIN}
											{#foreach $T.list as Result}
												<tr class="gradeX" id="gradeX{$T.Result.propsSid}">
													<td align="center" style="vertical-align:middle;cursor:pointer;" onclick="spanTd({$T.Result.propsSid},{$T.Result.categorySid},{$T.Result.channelSid})">
													...
													<td id="roleName_{$T.Result.sid}" style="vertical-align:middle;">
...
<!-- 												<input type="hidden" id="divShopSid" name="shopSid"> -->
												...
												<div class="form-group" id="entryCodeDiv2" style="display:none">
													分类编码： <input type="text" placeholder="必填"
											...
											<form id="div2Form" method="post" class="form-horizontal"
												...
												<div class="col-md-12">
													...
													<div class="col-lg-12">
														<span class="input-icon icon-right"> <input
															...
															id="name2" name="name" class="form-control" style="width:100%;"> <i
															class="glyphicon glyphicon-user danger"></i>
												...
												<div class="col-md-12">
													...
													<div class="col-lg-12">
														...
														<!-- 自定义滚动条 -->
														<table id="headtitle" class="mt15 table table-bordered">
