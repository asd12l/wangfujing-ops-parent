...
public class ValidateLengthMin  extends AbstractValidator{

	public void doValidator(List<Field> fields, List<String> args, boolean isAll) {
		if(args.size()>0 ){
			int minLen = Integer.parseInt(args.get(0));
			for (int i = 0; i < fields.size(); i++) {
				if(i==0 || result || isAll){
					Field field = fields.get(i);
					if(field.getValue()!=null && !"".equals(field.getValue())){
						if(field.getValue().toString().length()<minLen){
							result = false;
							msg +=field.getFieldDesc()+"：长度不能小于 "+minLen+" !";	
						}
					}
				}
			}	
